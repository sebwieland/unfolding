...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	cout << "(1) Unfolding using no Regularisation and no Area Constraint" << endl;

	TUnfoldDensity* unfold1 = new TUnfoldDensity(MigrationMatrix, TUnfold::kHistMapOutputVert, TUnfold::kRegModeNone,
	        TUnfold::kEConstraintNone, TUnfoldDensity::kDensityModeBinWidthAndUser, 0, 0, 0, "*[UOB]" );
	unfold1->SetInput(Data);
	unfold1->DoUnfold(0, Data);
	TH1* unfolded1 = unfold1->GetOutput("unfolded1");
	DrawDataMC(unfolded1, {Gen}, {"Gen"}, "UnfoldedvsGen1", false, false, "Pt", "#events");

	TH2* ErrorMatrix1 = unfold1->GetEmatrixTotal("ErrorMatrix1");
	Draw2D(ErrorMatrix1, "CovMatrix1", false);
	TH1D* UnfoldedTotalError1 = new TH1D("TotalError1", "Pt", nBins_Gen, xmin, xmax);
	for (Int_t bin = 1; bin <= nBins_Gen; bin++) {
		UnfoldedTotalError1->SetBinContent(bin, unfolded1->GetBinContent(bin));
		UnfoldedTotalError1->SetBinError(bin, sqrt(ErrorMatrix1->GetBinContent(bin, bin)));
	}
	DrawDataMC(UnfoldedTotalError1, {Gen}, {"Gen"}, "UnfoldedEvsGen1", false, false, "Pt", "#events");
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	cout << "(2) Unfolding using size Regularisation and no Area Constraint" << endl;

	TUnfoldDensity* unfold2 = new TUnfoldDensity(MigrationMatrix, TUnfold::kHistMapOutputVert, TUnfold::kRegModeSize,
	        TUnfold::kEConstraintNone, TUnfoldDensity::kDensityModeBinWidthAndUser, 0, 0, 0, "*[UOB]" );
	...
	TGraph *lCurve2avg = 0;
	iBest2avg = unfold2->ScanTau(nScan, tauMin, tauMax, &scanResult2avg, TUnfoldDensity::kEScanTauRhoAvgSys, 0, 0, &lCurve2avg);
	...
	DrawDataMC(unfolded2, {Gen}, {"Gen"}, "UnfoldedvsGen2_avgtau", false, false, "Pt", "#events");

	TH2* ErrorMatrix2 = unfold2->GetEmatrixTotal("ErrorMatrix2");
	Draw2D(ErrorMatrix2, "CovMatrix2_avgtau", false);
	TH1D* UnfoldedTotalError2 = new TH1D("TotalError2", "Pt", nBins_Gen, xmin, xmax);
	for (Int_t bin = 1; bin <= nBins_Gen; bin++) {
		UnfoldedTotalError2->SetBinContent(bin, unfolded2->GetBinContent(bin));
		UnfoldedTotalError2->SetBinError(bin, sqrt(ErrorMatrix2->GetBinContent(bin, bin)));
	}
	DrawDataMC(UnfoldedTotalError2, {Gen}, {"Gen"}, "UnfoldedEvsGen2_avgtau", false, false, "Pt", "#events");
	...
	TGraph *lCurve2max = 0;
	iBest2max = unfold2->ScanTau(nScan, tauMin, tauMax, &scanResult2max, TUnfoldDensity::kEScanTauRhoMax, 0, 0, &lCurve2max);
	std::cout << " Best tau=" << unfold2->GetTau() << "\n";
	...
	DrawDataMC(unfolded2max, {Gen}, {"Gen"}, "UnfoldedvsGen2_maxtau", false, false, "Pt", "#events");

	TH2* ErrorMatrix2max = unfold2->GetEmatrixTotal("ErrorMatrix2max");
	Draw2D(ErrorMatrix2max, "CovMatrix2_maxtau", false);
	TH1D* UnfoldedTotalError2max = new TH1D("TotalError2max", "Pt", nBins_Gen, xmin, xmax);
	for (Int_t bin = 1; bin <= nBins_Gen; bin++) {
		UnfoldedTotalError2max->SetBinContent(bin, unfolded2->GetBinContent(bin));
		UnfoldedTotalError2max->SetBinError(bin, sqrt(ErrorMatrix2->GetBinContent(bin, bin)));
	}
	DrawDataMC(UnfoldedTotalError2max, {Gen}, {"Gen"}, "UnfoldedEvsGen2_maxtau", false, false, "Pt", "#events");
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	cout << "(3) Unfolding using 1st Derivative Regularisation and no Area Constraint" << endl;

	TUnfoldDensity* unfold3 = new TUnfoldDensity(MigrationMatrix, TUnfold::kHistMapOutputVert, TUnfold::kRegModeDerivative,
	        TUnfold::kEConstraintNone, TUnfoldDensity::kDensityModeBinWidthAndUser, 0, 0, 0, "*[UOB]" );
	...
	TGraph *lCurve3avg = 0;
	iBest3avg = unfold3->ScanTau(nScan, tauMin, tauMax, &scanResult3avg, TUnfoldDensity::kEScanTauRhoAvgSys, 0, 0, &lCurve3avg);
	...
	DrawDataMC(unfolded3, {Gen}, {"Gen"}, "UnfoldedvsGen3_avgtau", false, false, "Pt", "#events");

	TH2* ErrorMatrix3 = unfold3->GetEmatrixTotal("ErrorMatrix3");
	Draw2D(ErrorMatrix3, "CovMatrix3_avgtau", false);
	TH1D* UnfoldedTotalError3 = new TH1D("TotalError3", "Pt", nBins_Gen, xmin, xmax);
	for (Int_t bin = 1; bin <= nBins_Gen; bin++) {
		UnfoldedTotalError3->SetBinContent(bin, unfolded3->GetBinContent(bin));
		UnfoldedTotalError3->SetBinError(bin, sqrt(ErrorMatrix3->GetBinContent(bin, bin)));
	}
	DrawDataMC(UnfoldedTotalError3, {Gen}, {"Gen"}, "UnfoldedEvsGen3_avgtau", false, false, "Pt", "#events");
	...
	// tauMin=0.0;
	// tauMax=0;
	iBest3max = unfold3->ScanTau(nScan, tauMin, tauMax, &scanResult3max, TUnfoldDensity::kEScanTauRhoMaxSys, 0, 0, &lCurve3max);
	std::cout << " Best tau=" << unfold3->GetTau() << "\n";
	...
	DrawDataMC(unfolded3max, {Gen}, {"Gen"}, "UnfoldedvsGen3_maxtau", false, false, "Pt", "#events");

	TH2* ErrorMatrix3max = unfold3->GetEmatrixTotal("ErrorMatrix3max");
	Draw2D(ErrorMatrix3max, "CovMatrix3_maxtau", false);
	TH1D* UnfoldedTotalError3max = new TH1D("TotalError3max", "Pt", nBins_Gen, xmin, xmax);
	for (Int_t bin = 1; bin <= nBins_Gen; bin++) {
		UnfoldedTotalError3max->SetBinContent(bin, unfolded3->GetBinContent(bin));
		UnfoldedTotalError3max->SetBinError(bin, sqrt(ErrorMatrix3->GetBinContent(bin, bin)));
	}
	DrawDataMC(UnfoldedTotalError3max, {Gen}, {"Gen"}, "UnfoldedEvsGen3_maxtau", false, false, "Pt", "#events");
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	cout << "(4) Unfolding using 2nd Derivative Regularisation and no Area Constraint" << endl;

	TUnfoldDensity* unfold4 = new TUnfoldDensity(MigrationMatrix, TUnfold::kHistMapOutputVert, TUnfold::kRegModeCurvature,
	        TUnfold::kEConstraintNone, TUnfoldDensity::kDensityModeBinWidthAndUser, 0, 0, 0, "*[UOB]" );
	...
	TGraph *lCurve4avg = 0;
	iBest4avg = unfold4->ScanTau(nScan, tauMin, tauMax, &scanResult4avg, TUnfoldDensity::kEScanTauRhoAvgSys, 0, 0, &lCurve4avg);
	...
	DrawDataMC(unfolded4, {Gen}, {"Gen"}, "UnfoldedvsGen4_avgtau", false, false, "Pt", "#events");

	TH2* ErrorMatrix4 = unfold4->GetEmatrixTotal("ErrorMatrix4");
	Draw2D(ErrorMatrix4, "CovMatrix4_avgtau", false);
	TH1D* UnfoldedTotalError4 = new TH1D("TotalError4", "Pt", nBins_Gen, xmin, xmax);
	for (Int_t bin = 1; bin <= nBins_Gen; bin++) {
		UnfoldedTotalError4->SetBinContent(bin, unfolded4->GetBinContent(bin));
		UnfoldedTotalError4->SetBinError(bin, sqrt(ErrorMatrix4->GetBinContent(bin, bin)));
	}
	DrawDataMC(UnfoldedTotalError4, {Gen}, {"Gen"}, "UnfoldedEvsGen4_avgtau", false, false, "Pt", "#events");
	...
	tauMin = 0.0;
	tauMax = 0;
	iBest4max = unfold4->ScanTau(nScan, tauMin, tauMax, &scanResult4max, TUnfoldDensity::kEScanTauRhoMaxSys, 0, 0, &lCurve4max);
	std::cout << " Best tau=" << unfold4->GetTau() << "\n";
	...
	DrawDataMC(unfolded4max, {Gen}, {"Gen"}, "UnfoldedvsGen4_maxtau", false, false, "Pt", "#events");

	TH2* ErrorMatrix4max = unfold4->GetEmatrixTotal("ErrorMatrix4max");
	Draw2D(ErrorMatrix4max, "CovMatrix4_maxtau", false);
	TH1D* UnfoldedTotalError4max = new TH1D("TotalError4max", "Pt", nBins_Gen, xmin, xmax);
	for (Int_t bin = 1; bin <= nBins_Gen; bin++) {
		UnfoldedTotalError4max->SetBinContent(bin, unfolded4->GetBinContent(bin));
		UnfoldedTotalError4max->SetBinError(bin, sqrt(ErrorMatrix4->GetBinContent(bin, bin)));
	}
	DrawDataMC(UnfoldedTotalError4max, {Gen}, {"Gen"}, "UnfoldedEvsGen4_maxtau", false, false, "Pt", "#events");
