...
void DrawRatio(TH1* hist1, TH1* hist2, TString name, TString xlabel,
               TString ylabel) {
  TFile* output = new TFile("output.root", "update");
  TCanvas* c = getCanvas(name, false);
...
void DrawDataMC(TH1* data, std::vector<TH1*> MC, std::vector<std::string> names,
                TString name, bool log, bool normalize, TString xlabel,
                TString ylabel) {
  TFile* output = new TFile("output.root", "update");
  ...
  c->SaveAs("plots/" + name + "_stacked.pdf");
  c->SaveAs("plots/" + name + "_stacked.png");
  c->Write();
  output->Close();
}

void VisualizeTau(int nScan, int iBest, TSpline* scanResult, TGraph* lCurve,
                  TString name) {
  ...
  bestRhoLogTau->Draw("*");
  tau->SaveAs("plots/" + name + ".pdf");
  tau->SaveAs("plots/" + name + ".png");
  tau->Write();
  output->Close();
}
